...
<\body>
  ...
  <\render-exercise|2.1-3>
    ...
    <\description>
      ...
      <item*|Output>An index <math|i> such that <math|v=A<around*|[|i|]>> or
      the special value <with|font-shape|small-caps|nil> if <math|v> does not
      appear in <math|A>.
    </description>

    Write pseudocode for <strong|<em|linear search>>, which scans through the
    sequence, looking for <math|v>. Using a loop invariant, prove that your
    ...
    <\answer>
      ...
      The pseudocode called <with|font-shape|small-caps|Linear-Search> scans
      through the sequence, looking for the first element equals to <math|v>,
      it returns <with|font-shape|small-caps|nil> if no match occurs in this
      process.
      ...
      <\render-code>
        ...
        3<hspace|8ex><strong|if> <math|A<around*|[|i|]> <text|<verbatim|==>>
        v>
      ...
      Observe that before entering each iteration of the <strong|while> loop,
      which is indexed by <math|i>, the subarray consisting of elements
      <math|A<around*|[|1 . . i-1|]>> consistutes the set not containing
      <math|v>, and the remaining subarray <math|A<around*|[|i . . n|]>>
      corresponds to the sequence to be search. This reveals the following
      loop invariant:

      <\indent>
        At the start of each iteration of the <strong|while> loop of line
        2--5, the element of value <math|v> does not belongs to the subarray
        <math|A<around*|[|1 . . i-1|]>>.
      </indent>

      <hspace|3ex>We use this loop invariant to help us prove the correctness
      of <with|font-shape|small-caps|Linear-Search>.

      <\description>
        <item*|Initialization>Prior to the first iteration of the loop, we
        have <math|i=1>, so that the subarray <math|A<around*|[|1 . . i-1|]>>
        is empty. This empty subarray does not contain the element of value
        <math|v>.
        ...
        element of value <math|v> or run out of the array. In the first case,
        after passing by <math|i-1> elements that are not equal to <math|v>,
        the element that terminate the loop is the <math|i>th one, and this
        index of <math|i> is returned. In second case, we must have
        <math|i=n+1> at that time. Substituting <math|n+1> for <math|i> in
        the statement of loop invariant, we have that the element of value
        <math|v> does not belongs to the subarray <math|A<around*|[|1 . .
        n|]>>. Observing that the subarray <math|A<around*|[|1 . . n|]>> is
        the entire array, we conclude that <math|v> does not appear in
        <math|A>, and return the special value
