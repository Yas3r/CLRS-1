<TeXmacs|1.99.1>

<style|generic>

<\body>
  <\render-exercise|4.4-1>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    Use a recursion tree to determine a good asymptotic upper bound on the
    recurrence <math|T <around*|(|n|)>=3*T
    <around*|(|<around*|\<lfloor\>|n/2|\<rfloor\>>|)>+n>. Use the
    substitution method to verify your answer.

    <\answer>
      \;

      We omit the floor function for simplicity and create a recursion tree
      for the recurrence <math|T <around*|(|n|)>=3*T <around*|(|n/2|)>+n>,
      which is shown in Figure <\float|float|tbh>
        <big-figure|<with|gr-mode|<tuple|group-edit|move>|gr-frame|<tuple|scale|1cm|<tuple|426290tmpt|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|cartesian|<point|0|0>|1>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|<graphics|>>|>
      </float>.<new-line> <hspace|2ex>Because subproblem sizes, decrease by a
      factor of <math|2> each step we go down the tree, we eventually must
      reach a boundary condition. The subproblem size for a node at depth
      <math|i> is <math|n/2<rsup|i>>. Thus, the subproblem size hits <math|T
      <around*|(|n|)>=1> when <math|n/2<rsup|i>=1> or, equivalently, when
      <math|i=log<rsub|2> n>. Thus, the tree has <math|log<rsub|2> n+1>
      levels (at depth <math|0,1,2,\<ldots\>,log<rsub|2> n>).<new-line>
      <hspace|2ex>Next we determine the cost at each level of the tree. Each
      level has three times more nodes than the level above, and so the
      number of nodes at depth <math|i> is <math|3<rsup|i>>. Because
      subproblem sizes reduce by a factor of <math|2> for aech level we go
      down from the root, each node at depth <math|i>, for
      <math|i=0,1,2,\<ldots\>,log<rsub|2> n-1>, has a cost of
      <math|n/2<rsup|i>>. Multiplying, we see that total the cost over all
      nodes at depth <math|i>, is <math|<around*|(|3/2|)><rsup|i>*n>. The
      bottom level, at depth <math|log<rsub|2> n>, has
      <math|3<rsup|log<rsub|2> n>=n<rsup|log<rsub|2> 3>> nodes, each
      contributing cost <math|T <around*|(|1|)>>, for a total cost of
      <math|n<rsup|log<rsub|2> 3>*T <around*|(|1|)>>, which is
      <math|\<Theta\> <around*|(|n<rsup|log<rsub|2> 3>|)>>, for we assume
      that <math|T <around*|(|1|)>> is a constant.<new-line> <hspace|2ex>Now
      we add up the costs over all levels to determine the cost for the
      entire tree:

      <\eqnarray*>
        <tformat|<table|<row|<cell|T <around*|(|n|)>>|<cell|=>|<cell|n+<frac|3|2>*n+<around*|(|<frac|3|2>|)><rsup|2>*n+\<cdots\>+<around*|(|<frac|3|2>|)><rsup|log<rsub|2>
        n-1>*n+\<Theta\> <around*|(|n<rsup|log<rsub|2>
        3>|)>>>|<row|<cell|>|<cell|=>|<cell|<above|<below|<big|sum>|i=0>|log<rsub|2>
        n-1><around*|(|<frac|3|2>|)><rsup|i>*n+\<Theta\>
        <around*|(|n<rsup|log<rsub|2> 3>|)>>>|<row|<cell|>|<cell|=>|<cell|<frac|<around*|(|3/2|)><rsup|log<rsub|2>
        n>-1|<around*|(|3/2|)>-1>*n+\<Theta\> <around*|(|n<rsup|log<rsub|2>
        3>|)>>>|<row|<cell|>|<cell|=>|<cell|2*<around*|[|<around*|(|3/2|)><rsup|log<rsub|2>
        n>-1|]>*n+\<Theta\> <around*|(|n<rsup|log<rsub|2>
        3>|)>>>|<row|<cell|>|<cell|=>|<cell|2*<around*|[|n<rsup|log<rsub|2>
        <around*|(|3/2|)>>-1|]>*n+\<Theta\> <around*|(|n<rsup|log<rsub|2>
        3>|)>>>|<row|<cell|>|<cell|\<less\>>|<cell|2*<around*|(|n<rsup|log<rsub|2>
        2>-1|)>*n+\<Theta\> <around*|(|n<rsup|log<rsub|2>
        3>|)>>>|<row|<cell|>|<cell|=>|<cell|2*<around*|(|n-1|)>*n+\<Theta\>
        <around*|(|n<rsup|log<rsub|2> 3>|)>>>|<row|<cell|>|<cell|=>|<cell|2*n<rsup|2>-2*n+\<Theta\>
        <around*|(|n<rsup|log<rsub|2> 3>|)>>>|<row|<cell|>|<cell|=>|<cell|O
        <around*|(|n<rsup|2>|)>>>>>
      </eqnarray*>

      <hspace|3ex>Indeed, we can use the substitution method to verify that
      <math|O <around*|(|n<rsup|2>|)>> is an upper bound for the solution to
      the recurrence. We show that <math|T
      <around*|(|n|)>\<leq\>c*n<rsup|2>-d*n>, where <math|c> and <math|d> are
      both suitable positive constants. We have

      <\eqnarray*>
        <tformat|<table|<row|<cell|T <around*|(|n|)>>|<cell|\<leq\>>|<cell|3*T
        <around*|(|n/2|)>+n>>|<row|<cell|>|<cell|\<leq\>>|<cell|3*c*<around*|(|n/2|)><rsup|2>-d*<around*|(|n/2|)>+n>>|<row|<cell|>|<cell|=>|<cell|<around*|(|3/4|)>*c*n<rsup|2>-<around*|(|d/2|)>*n+n>>|<row|<cell|>|<cell|\<leq\>>|<cell|c*n<rsup|2>-d*n>>>>
      </eqnarray*>

      where the last step holds as longs <math|d\<gtr\>-2>.\ 
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|exercise-indentation|0tab>
    <associate|font-base-size|12>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|auto-1|<tuple|1|?|../../../../../../../../../../.TeXmacs/texts/scratch/no_name_30.tm>>
    <associate|footnote-|<tuple|?|?|../../../../../../../../../../.TeXmacs/texts/scratch/no_name_30.tm>>
    <associate|footnote-*|<tuple|?|?|../../../../../../../../../../.TeXmacs/texts/scratch/no_name_30.tm>>
  </collection>
</references>