<TeXmacs|1.99.1>

<style|generic>

<\body>
  <\render-exercise|10.3-4>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    It is often desirable to keep all elements of a doubly linked list
    compact in storage, using, for example, the first <math|m> index
    locations in the multiple-array representation. (This is the case in the
    paged, virtual-memory computing environment.) Explain how to impoement
    the procedures <with|font-shape|small-caps|Allocate-Object> and
    <with|font-shape|small-caps|Free-Object> so that the representation is
    compact. Assume that there are no pointers to elements of linked list
    outside the list itself. (<em|Hint:> Use the array implementation of a
    stack.)

    <\answer>
      \;

      Suppose that at some moment the dynamic set contains <math|m\<leq\>n>
      elements and they are stored compact. Then the first <math|m> objects
      represent elements currently in the dynamic set, and the remaining
      <math|n-m> objects are free.<new-line> <hspace|2ex>We keep the free
      objects in an array implementation of a stack
      <math|F<around*|[|F.<text|<em|top>>..m|]>>, where
      <math|F<around*|[|m|]>> is the element at the bottom of the stack and
      <math|F<around*|[|F.<text|<em|top>>|]>> is the element at the top. The
      free list uses an array implementation of the stack operations
      <with|font-shape|small-caps|Push> and <with|font-shape|small-caps|Pop>
      to implement the procedures for allocating and freeing objects,
      respectively. We assume that the global variable <math|F> used in the
      following procedures points to the stack <math|F>.

      <\render-code>
        <with|font-shape|small-caps|Allocate-Object><math|<around*|(||)>>

        1<hspace|3ex><strong|if> <with|font-shape|small-caps|Stack-Empty><math|<around*|(|F|)>>

        2<hspace|8ex><strong|error> ``out of space''

        3<hspace|3ex><strong|else> <math|x> =
        <with|font-shape|small-caps|Pop><math|<around*|(|F|)>>

        4<hspace|8ex><strong|return> <math|x>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Free-Object><math|<around*|(|x|)>>

        1<hspace|3ex><em|p> <math|=> <em|F.top> <math|-> <math|1>

        2<hspace|3ex><em|p.prev.next> <math|=> <math|x>

        3<hspace|3ex><em|p.next.prev> <math|=> <math|x>

        4<hspace|3ex><em|x.key> <math|=> <em|p.key>

        5<hspace|3ex><em|x.prev> <math|=> <em|p.prev>

        6<hspace|3ex><em|x.next> <math|=> <em|p.next>

        7<hspace|3ex><with|font-shape|small-caps|Push><math|<around*|(|F,p|)>>
      </render-code>

      The free list initially contains all <math|m> unallocated objects. Once
      the free list has been exhausted, running the
      <with|font-shape|small-caps|Allocate-Object> procedure signals an
      error. To free an object at index <math|x>, we first override this slot
      by the object <math|p> left to <em|F.top> in the doubly linked list,
      then join <math|p> into the free list, as Figure
      <reference|10.3-4_Figure><\float|float|tbh>
        <\big-figure>
          <with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-auto-crop|true|gr-arrow-end|\<gtr\>|<graphics||<gr-group|<with|arrow-end|\<gtr\>|<line|<point|0.45|0>|<point|0.45|-0.7>|<point|-1.6|-0.7>|<point|-1.6|0>>>|<with|arrow-end|\<gtr\>|<line|<point|-1.35|0>|<point|-1.35|-0.9>|<point|-0.65|-0.9>|<point|-0.65|0.0>>>|<text-at|<em|prev>|<point|-2.58105882703239|0.2>>|<with|arrow-end|\<gtr\>|<line|<point|-0.2|0>|<point|-0.2|-0.3>|<point|-1.1|-0.3>|<point|-1.1|0>>>|<text-at|<em|key>|<point|-2.455163|0.5952342>>|<with|arrow-end|\<gtr\>|<line|<point|-0.9|0.0>|<point|-0.9|-0.5>|<point|0.25|-0.5>|<point|0.25|0.0>>>|<text-at|<em|next>|<point|-2.570495|1.009072>>|<line|<point|-0.7|0.1>|<point|-0.425|0.35>>|<with|arrow-end|\<gtr\>|<line|<point|-0.4|1.4>|<point|-0.4|2.3>|<point|-1.6|2.3>|<point|-1.6|1.4>>>|<cline|<point|-3.0|2.75>|<point|-2.5|2.75>|<point|-2.5|2.25>|<point|-3.0|2.25>>|<with|arrow-end|\<gtr\>|<line|<point|-1.35|1.4>|<point|-1.35|2.1>|<point|0.45|2.1>|<point|0.45|1.4>>>|<text-at|<math|L>|<point|-3.5|2.4>>|<with|arrow-end|\<gtr\>|<line|<point|0.25|1.4>|<point|0.25|1.9>|<point|-1.1|1.9>|<point|-1.1|1.4>>>|<text-at|3|<point|-2.825|2.4>>|<with|arrow-end|\<gtr\>|<line|<point|0.8|-1>|<point|0.8|0.0>>>|<with|arrow-end|\<gtr\>|<line|<point|-0.9|1.4>|<point|-0.9|1.7>|<point|-0.2|1.7>|<point|-0.2|1.4>>>|<with|arrow-end|\<gtr\>|<line|<point|-2.5|2.5>|<point|-0.65|2.5>|<point|-0.65|1.4>>>|<line|<point|-0.25|1>|<point|0|1.25>>|<text-at|<em|F.top>
          = 6|<point|0|-1.4>>|<text-at|<very-small|1>|<point|-1.5|2.7>>|<text-at|<very-small|2>|<point|-1.05|2.7>>|<text-at|<very-small|3>|<point|-0.6|2.7>>|<text-at|<very-small|4>|<point|-0.15|2.7>>|<text-at|<very-small|5>|<point|0.3|2.7>>|<text-at|<very-small|6>|<point|0.75|2.7>>|<text-at|<very-small|7>|<point|1.2|2.7>>|<text-at|<block*|<tformat|<cwith|1|-1|1|-1|cell-hyphen|t>|<cwith|2|2|10|10|cell-hyphen|n>|<cwith|1|-1|1|-1|cell-width|3ex>|<cwith|1|-1|1|-1|cell-hmode|exact>|<cwith|1|-1|1|-1|cell-height|3ex>|<cwith|1|-1|1|-1|cell-vmode|exact>|<cwith|1|-1|1|-1|cell-lborder|0.5mm>|<cwith|1|-1|10|10|cell-rborder|0.5mm>|<cwith|1|1|1|-1|cell-tborder|0.5mm>|<cwith|3|3|1|-1|cell-bborder|0.5mm>|<table|<row|<\cell>
            5
          </cell>|<\cell>
            4
          </cell>|<\cell>
            1
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            2
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>>|<row|<\cell>
            8
          </cell>|<\cell>
            7
          </cell>|<\cell>
            5
          </cell>|<\cell>
            11
          </cell>|<\cell>
            19
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<cell|>>|<row|<\cell>
            3
          </cell>|<\cell>
            5
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            2
          </cell>|<\cell>
            1
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>>>>>|<point|-1.7|0.6>>|<text-at|<very-small|8>|<point|1.65|2.7>>|<text-at|<math|F>|<point|3.0|1.0>>|<text-at|<very-small|9>|<point|2.1|2.7>>|<text-at|<very-small|10>|<point|2.5|2.7>>>>><hspace|5ex><with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-auto-crop|true|gr-arrow-end|\<gtr\>|<graphics||<gr-group|<with|arrow-end|\<gtr\>|<line|<point|0.45|0>|<point|0.45|-0.5>|<point|-1.6|-0.5>|<point|-1.6|0>>>|<text-at|<em|prev>|<point|-2.58105882703239|0.2>>|<with|arrow-end|\<gtr\>|<line|<point|-1.35|0>|<point|-1.35|-0.3>|<point|-0.65|-0.3>|<point|-0.65|0.0>>>|<text-at|<em|key>|<point|-2.455163|0.5952342>>|<text-at|<em|next>|<point|-2.570495|1.009072>>|<line|<point|-0.7|0.1>|<point|-0.425|0.35>>|<cline|<point|-3.0|2.75>|<point|-2.5|2.75>|<point|-2.5|2.25>|<point|-3.0|2.25>>|<with|arrow-end|\<gtr\>|<line|<point|-0.4|1.4>|<point|-0.4|1.7>|<point|-1.6|1.7>|<point|-1.6|1.4>>>|<text-at|<math|L>|<point|-3.5|2.4>>|<with|arrow-end|\<gtr\>|<line|<point|-0.2|0>|<point|-0.2|-0.3>|<point|0.2|-0.3>|<point|0.2|0.0>>>|<with|arrow-end|\<gtr\>|<line|<point|-1.35|1.4>|<point|-1.35|1.9>|<point|0.45|1.9>|<point|0.45|1.4>>>|<text-at|3|<point|-2.825|2.4>>|<with|arrow-end|\<gtr\>|<line|<point|-2.5|2.5>|<point|-0.65|2.5>|<point|-0.65|1.4>>>|<line|<point|-0.25|1>|<point|0|1.25>>|<with|arrow-end|\<gtr\>|<line|<point|0.8|-1>|<point|0.8|0.0>>>|<with|arrow-end|\<gtr\>|<line|<point|0.25|1.4>|<point|0.25|1.7>|<point|0.0|1.7>|<point|0.0|1.4>>>|<text-at|<very-small|1>|<point|-1.5|2.7>>|<text-at|<em|F.top>
          = 6|<point|0|-1.4>>|<text-at|<very-small|2>|<point|-1.05|2.7>>|<text-at|<very-small|3>|<point|-0.6|2.7>>|<text-at|<very-small|4>|<point|-0.15|2.7>>|<text-at|<very-small|5>|<point|0.3|2.7>>|<text-at|<very-small|6>|<point|0.75|2.7>>|<text-at|<very-small|7>|<point|1.2|2.7>>|<text-at|<block*|<tformat|<cwith|1|-1|1|-1|cell-hyphen|t>|<cwith|2|2|10|10|cell-hyphen|n>|<cwith|1|-1|1|-1|cell-width|3ex>|<cwith|1|-1|1|-1|cell-hmode|exact>|<cwith|1|-1|1|-1|cell-height|3ex>|<cwith|1|-1|1|-1|cell-vmode|exact>|<cwith|1|-1|1|-1|cell-lborder|0.5mm>|<cwith|1|-1|10|10|cell-rborder|0.5mm>|<cwith|1|1|1|-1|cell-tborder|0.5mm>|<cwith|3|3|1|-1|cell-bborder|0.5mm>|<table|<row|<\cell>
            5
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            1
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            4
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>>|<row|<\cell>
            8
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            5
          </cell>|<\cell>
            11
          </cell>|<\cell>
            19
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<cell|>>|<row|<\cell>
            3
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            5
          </cell>|<\cell>
            1
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>>>>>|<point|-1.7|0.6>>|<text-at|<very-small|8>|<point|1.65|2.7>>|<text-at|<math|F>|<point|3.0|1.0>>|<text-at|<very-small|9>|<point|2.1|2.7>>|<text-at|<very-small|10>|<point|2.5|2.7>>>>>

          \;

          <hspace|26ex>(a)<hspace|48ex>(b)

          \;

          \;

          <with|gr-mode|<tuple|group-edit|group-ungroup>|gr-frame|<tuple|scale|1cm|<tuple|0.5gw|0.5gh>>|gr-geometry|<tuple|geometry|1par|0.6par>|gr-grid|<tuple|empty>|gr-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-edit-grid-aspect|<tuple|<tuple|axes|none>|<tuple|1|none>|<tuple|10|none>>|gr-edit-grid|<tuple|empty>|gr-edit-grid-old|<tuple|cartesian|<point|0|0>|1>|gr-auto-crop|true|gr-arrow-end|\<gtr\>|<graphics||<gr-group|<with|arrow-end|\<gtr\>|<line|<point|-0.9|0.0>|<point|-0.9|-0.5>|<point|-1.6|-0.5>|<point|-1.6|0>>>|<with|arrow-end|\<gtr\>|<line|<point|-1.35|0>|<point|-1.35|-0.7>|<point|-0.65|-0.7>|<point|-0.65|0.0>>>|<text-at|<em|prev>|<point|-2.58105882703239|0.2>>|<with|arrow-end|\<gtr\>|<line|<point|-0.2|0>|<point|-0.2|-0.3>|<point|-1.1|-0.3>|<point|-1.1|0>>>|<text-at|<em|key>|<point|-2.455163|0.5952342>>|<text-at|<em|next>|<point|-2.570495|1.009072>>|<line|<point|-0.7|0.1>|<point|-0.425|0.35>>|<with|arrow-end|\<gtr\>|<line|<point|-0.45|1.4>|<point|-0.45|1.9>|<point|-1.6|1.9>|<point|-1.6|1.4>>>|<with|arrow-end|\<gtr\>|<line|<point|0.3|-1.0>|<point|0.3|0.0>>>|<cline|<point|-3.0|2.75>|<point|-2.5|2.75>|<point|-2.5|2.25>|<point|-3.0|2.25>>|<with|arrow-end|\<gtr\>|<line|<point|-1.35|1.4>|<point|-1.35|1.7>|<point|-1.1|1.7>|<point|-1.1|1.4>>>|<text-at|<math|L>|<point|-3.5|2.4>>|<text-at|3|<point|-2.825|2.4>>|<text-at|<em|F.top>
          = 5|<point|-0.5|-1.4>>|<with|arrow-end|\<gtr\>|<line|<point|-0.9|1.4>|<point|-0.9|1.7>|<point|-0.2|1.7>|<point|-0.2|1.4>>>|<with|arrow-end|\<gtr\>|<line|<point|-2.5|2.5>|<point|-0.65|2.5>|<point|-0.65|1.4>>>|<line|<point|-0.25|1>|<point|0|1.25>>|<text-at|<very-small|1>|<point|-1.5|2.7>>|<text-at|<very-small|2>|<point|-1.05|2.7>>|<text-at|<very-small|3>|<point|-0.6|2.7>>|<text-at|<very-small|4>|<point|-0.15|2.7>>|<text-at|<very-small|5>|<point|0.3|2.7>>|<text-at|<very-small|6>|<point|0.75|2.7>>|<text-at|<very-small|7>|<point|1.2|2.7>>|<text-at|<block*|<tformat|<cwith|1|-1|1|-1|cell-hyphen|t>|<cwith|2|2|10|10|cell-hyphen|n>|<cwith|1|-1|1|-1|cell-width|3ex>|<cwith|1|-1|1|-1|cell-hmode|exact>|<cwith|1|-1|1|-1|cell-height|3ex>|<cwith|1|-1|1|-1|cell-vmode|exact>|<cwith|1|-1|1|-1|cell-lborder|0.5mm>|<cwith|1|-1|10|10|cell-rborder|0.5mm>|<cwith|1|1|1|-1|cell-tborder|0.5mm>|<cwith|3|3|1|-1|cell-bborder|0.5mm>|<table|<row|<\cell>
            2
          </cell>|<\cell>
            4
          </cell>|<\cell>
            1
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>>|<row|<\cell>
            8
          </cell>|<\cell>
            19
          </cell>|<\cell>
            5
          </cell>|<\cell>
            11
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<cell|>>|<row|<\cell>
            3
          </cell>|<\cell>
            1
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            2
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>|<\cell>
            \;
          </cell>>>>>|<point|-1.7|0.6>>|<text-at|<very-small|8>|<point|1.65|2.7>>|<text-at|<math|F>|<point|3.0|1.0>>|<text-at|<very-small|9>|<point|2.1|2.7>>|<text-at|<very-small|10>|<point|2.5|2.7>>>>>

          \;

          <hspace|26ex>(c)

          \;

          \;
        </big-figure|<label|10.3-4_Figure>The effect of the
        <with|font-shape|small-caps|Free-Object> procedures. <strong|(a)> The
        dynamic set <math|<around*|{|5,8,19,7,11|}>> represented by a doubly
        linked list, and a free list (first rows of empty slots) whose top
        element is at index 6. <strong|(b)> The result of deleting the object
        at index 2, leaving the storage of <math|L> truncated by an empty
        slot. <strong|(c)> After executing
        <with|font-shape|small-caps|Free-Object><math|<around*|(|2|)>>, the
        object at index 5 is reallocated to index 2, making <math|L> a
        compact list again.>
      </float> shows. Both procedures run in <math|O <around*|(|1|)>> time,
      which makes them quite efficient.
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|exercise-indentation|0tab>
    <associate|font-base-size|12>
    <associate|indent-indentation|0tab>
    <associate|preamble|false>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|10.3-4_Figure|<tuple|1|?>>
    <associate|auto-1|<tuple|1|2>>
    <associate|auto-2|<tuple|2|2>>
    <associate|footnote-|<tuple|?|?|../../../../../../../../../../.TeXmacs/texts/scratch/no_name_30.tm>>
    <associate|footnote-*|<tuple|?|1>>
  </collection>
</references>

<\auxiliary>
  <\collection>
    <\associate|figure>
      <tuple|normal||<pageref|auto-1>>
    </associate>
  </collection>
</auxiliary>