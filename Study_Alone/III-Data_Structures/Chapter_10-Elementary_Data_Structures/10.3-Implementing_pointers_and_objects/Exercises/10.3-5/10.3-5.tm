<TeXmacs|1.99.1>

<style|generic>

<\body>
  <\hide-preamble>
    \;

    <assign|answer|<\macro|body>
      <render-answer|<answer-text>|<arg|body>>
    </macro>>

    <assign|indent|<\macro|body>
      <\surround||<right-flush>>
        <\indent*>
          <arg|body>
        </indent*>
      </surround>
    </macro>>

    <assign|render-code|<\macro|body>
      <\surround||<no-indent*>>
        <\padded*>
          <\with|par-first|0fn|par-par-sep|0fn|item-hsep|<macro|1tab>>
            <arg|body>
          </with>
        </padded*>
      </surround>
    </macro>>
  </hide-preamble>

  <\render-exercise|10.3-5>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    Let <math|L> be a doubly linked list of length <math|n> stored in arrays
    <em|key>, <em|prev>, and <em|next> of length <math|m>. Suppose that these
    arrays are managed by <with|font-shape|small-caps|Allocate-Object> and
    <with|font-shape|small-caps|Free-Object> procedures that keep a doubly
    linked free list <math|F>. Suppose further that of the <math|m> items,
    excatly <math|n> are on list <math|L> and <math|m-n> are on the free
    list. Write a procedure <with|font-shape|small-caps|Compactify-List><math|<around*|(|L,F|)>>
    that, given the list <math|L> and the free list <math|F>, moves the items
    in <math|L> so that they occupy array positions <math|1,2,\<ldots\>,n>
    and adjusts the free list <math|F> so that it remains correct, occupying
    array positions <math|n+1,n+2,\<ldots\>,m>. The running time of your
    procedure should be <math|\<Theta\> <around*|(|n|)>>, and it should use
    only a constant amount of extra space. Argue that your procedure is
    correct.

    <\answer>
      \;

      We represent the combination of arrays <em|key>, <em|prev>, and
      <em|next> by a multible-array <math|A>. Each object of <math|A>'s is
      either in list <math|L> or in the free list <math|F>, but not in both.
      The procedure <with|font-shape|small-caps|Compactify-List> transposes
      the first object in <math|L> with the first object in <math|A>, the
      second objects, ... until the list <math|L> is exhausted.

      <\render-code>
        <with|font-shape|small-caps|Compactify-List><math|<around*|(|L,F|)>>

        <hspace|1.1ex>1<hspace|3ex><with|font-shape|small-caps|Transpose><math|<around*|(|A<around*|[|L.<text|<em|head>>|]>,A<around*|[|1|]>|)>>

        <hspace|1.1ex>2<hspace|3ex><strong|if> <math|F.<text|<em|head>>>
        <verbatim|==> 1

        <hspace|1.1ex>3<hspace|8ex><math|F.<text|<em|head>>> <math|=>
        <math|L.<text|<em|head>>>

        <hspace|1.1ex>4<hspace|3ex><math|L.<text|<em|head>>> <math|=>
        <math|1>

        <hspace|1.1ex>5<hspace|3ex><math|l> <math|=>
        <math|A<around*|[|L.<text|<em|head>>|]>.<text|<em|next>>>

        <hspace|1.1ex>6<hspace|3ex><math|i> <math|=> <math|2>

        <hspace|1.1ex>7<hspace|3ex><strong|while> <math|l> <math|\<neq\>>
        <with|font-shape|small-caps|nil>

        <hspace|1.1ex>8<hspace|8ex><with|font-shape|small-caps|Transpose><math|<around*|(|A<around*|[|l|]>,A<around*|[|i|]>|)>>

        <hspace|1.1ex>9<hspace|8ex><strong|if> <math|F> <verbatim|==>
        <math|i>

        10<hspace|13ex><math|F> <math|=> <math|l>

        11<hspace|8ex><math|l> <math|=> <math|A<around*|[|l|]>.<text|<em|next>>>

        12<hspace|8ex><math|i> <math|=> <math|i+1>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Transpose><math|<around*|(|e<rsub|1>,e<rsub|2>|)>>

        1<hspace|3ex><with|font-shape|small-caps|Swap><math|<around*|(|e<rsub|1>.<text|<em|prev>>.<text|<em|next>>,e<rsub|2>.<text|<em|prev>>.<text|<em|next>>|)>>

        2<hspace|3ex><with|font-shape|small-caps|Swap><math|<around*|(|e<rsub|1>.<text|<em|prev>>,e<rsub|2>.<text|<em|prev>>|)>>

        3<hspace|3ex><with|font-shape|small-caps|Swap><math|<around*|(|e<rsub|1>.<text|<em|next>>.<text|<em|prev>>,e<rsub|2>.<text|<em|next>>.<text|<em|prev>>|)>>

        4<hspace|3ex><with|font-shape|small-caps|Swap><math|<around*|(|e<rsub|1>.<text|<em|next>>,e<rsub|2>.<text|<em|next>>|)>>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Swap><math|<around*|(|x,y|)>>

        1<hspace|3ex><em|temp> <math|=> <math|x>

        2<hspace|3ex><math|x> <math|=> <math|y>

        3<hspace|3ex><math|y> <math|=> <em|temp>
      </render-code>

      <hspace|3ex>This <with|font-shape|small-caps|Compactify-List> procedure
      takes time <math|\<Theta\> <around*|(|n|)>>, and it use only a constant
      amount of extra space.

      To prove that this algorithm is correct, observe that at any moment, we
      can devide <math|A> into two subarrays: <math|A<around*|[|1 . . i-1|]>>
      of compact leading objects in <math|L> and <math|A<around*|[|i . .
      m|]>> of objects to be compactify. This reveals the following loop
      invariant:

      <\indent>
        At the start of each iteration of the <strong|while> loop of line
        7--12, the objects <math|A<around*|[|1 . . i-1|]>> are the first
        <math|i-1> compact objects in list <math|L>.
      </indent>

      <hspace|3ex>This loop invariant should help us prove the correctness of
      <with|font-shape|small-caps|Compactify-List>. For convenience, we
      denote the sequence of objects in <math|L> by
      <math|L<rsub|1>,L<rsub|2>,\<ldots\>,L<rsub|n>>.

      <\description>
        <item*|Initialization>Before entering the iteration of the
        <strong|while> loop, the procedure transpose <math|L<rsub|1>> with
        the first one in <math|A>, making <math|L>'s first object takes up
        <math|A<around*|[|1|]>>. It then increments <math|i> by <math|1> to
        be <math|i=2>, so that <math|A<around*|[|1 . .
        i-1|]>=A<around*|[|1|]>> is the only compact object in list <math|L>.
        If the free list <math|F> happens to be started at
        <math|A<around*|[|1|]>>, we update its head after this transposition.

        <item*|Maintenance>The loop continues as long as the list <math|L>
        has not been exhausted. It goes by transposing the remaining
        incompact objects <math|L<rsub|2>,L<rsub|3>,\<ldots\>,L<rsub|n>> of
        <math|L> and reallocates them at <math|A<around*|[|2|]>,A<around*|[|3|]>,\<ldots\>,A<around*|[|n|]>>.
        The compact sublist of <math|L> expands from <math|A<around*|[|1 . .
        i-1|]>> to <math|A<around*|[|1 . . i|]>>. Incrementing <math|i> for
        the next iteration of the <strong|while> loop then preserves the loop
        invariant. Still, if <math|F>'s head is encountered in this process,
        we update its pointer to the new position after transposition.\ 

        <item*|Termination>The condition cause the <strong|while> loop
        terminate is that <math|l=<text|<with|font-shape|small-caps|nil>>>.
        Because each loop iteration increase <math|i> by <math|1> and list
        <math|L> has a length <math|n>, we must have <math|i=n+1> at that
        moment. Substituting <math|n+1> for <math|i> in the statement of the
        loop invariant, we have that the objects <math|A<around*|[|1 . .
        n|]>> are the first <math|n> compact objects in list <math|L>. Since
        each object in array <math|A> is either in list <math|L> or in the
        free list <math|F>, the rest <math|m-n> continuous objects
        <math|A<around*|[|n+1|]>,A<around*|[|n+2|]>,\<ldots\>,A<around*|[|m|]>>
        must all belongs to the free list <math|F>. Hence, the algorithm is
        correct.
      </description>
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|exercise-indentation|0tab>
    <associate|font-base-size|12>
    <associate|indent-indentation|1tab>
    <associate|item-hsep|<macro|0fn>>
    <associate|preamble|false>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?>>
    <associate|footnote-*|<tuple|?|?>>
  </collection>
</references>