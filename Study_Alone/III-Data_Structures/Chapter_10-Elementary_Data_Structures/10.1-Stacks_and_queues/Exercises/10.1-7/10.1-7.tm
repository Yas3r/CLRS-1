<TeXmacs|1.99.1>

<style|generic>

<\body>
  <\render-exercise|10.1-7>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    Show how to implement a stack using two queues. Analyze the running time
    of the stack operations.

    <\answer>
      \;

      Implementing a stack is, in other words, maintaining the LIFO property
      on a sequence of inputs. To manage it with two queues <math|Q<rsub|1>>
      and <math|Q<rsub|2>>, we set one of them idle while storing all the
      stack elements in the other one. When an element is pushed into the
      stack <math|S>, it takes its place at the tail of the nonempty queue.
      The element pop from the stack <math|S> is always the last one in the
      nonempty queue. To extract this element, we first transfer all its
      predecessors to the empty queue, then dequeue it. A stack <math|S> is
      said to be empty if both its representaeive queues <math|Q<rsub|1>> and
      <math|Q<rsub|2>> are empty. Conversely, if either <math|Q<rsub|1>> or
      <math|Q<rsub|2>> are full, then <math|S> becomes full.

      <\render-code>
        <with|font-shape|small-caps|Stack-Empty><math|<around*|(|S|)>>

        <hspace|1.2ex>1<hspace|3ex><strong|if>
        <with|font-shape|small-caps|Queue-Empty><math|<around*|(|Q<rsub|1>|)>>
        and <with|font-shape|small-caps|Queue-Empty><math|<around*|(|Q<rsub|2>|)>>

        <hspace|1.2ex>2<hspace|8ex><strong|return>
        <with|font-shape|small-caps|true>

        <hspace|1.2ex>3<hspace|3ex><strong|else> <strong|return>
        <with|font-shape|small-caps|false>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Stack-Full><math|<around*|(|S|)>>

        <hspace|1.2ex>1<hspace|3ex><strong|if>
        <with|font-shape|small-caps|Queue-Full><math|<around*|(|Q<rsub|1>|)>>
        or <with|font-shape|small-caps|Queue-Full><math|<around*|(|Q<rsub|2>|)>>

        <hspace|1.2ex>2<hspace|8ex><strong|return>
        <with|font-shape|small-caps|true>

        <hspace|1.2ex>3<hspace|3ex><strong|else> <strong|return>
        <with|font-shape|small-caps|false>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Push><math|<around*|(|S,x|)>>

        <hspace|1.2ex>1<hspace|3ex><strong|if>
        <with|font-shape|small-caps|Stack-Full><math|<around*|(|S|)>>

        <hspace|1.2ex>2<hspace|8ex><strong|error> ``overflow''

        <hspace|1.2ex>3<hspace|3ex><strong|else>\ 

        <hspace|1.2ex>4<hspace|8ex><strong|if>
        <with|font-shape|small-caps|Queue-Empty><math|<around*|(|Q<rsub|2>|)>>

        <hspace|1.2ex>5<hspace|13ex><with|font-shape|small-caps|Enqueue><math|<around*|(|Q<rsub|1>,x|)>>

        <hspace|1.2ex>6<hspace|8ex><strong|else>
        <with|font-shape|small-caps|Enqueue><math|<around*|(|Q<rsub|2>,x|)>>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Pop><math|<around*|(|S|)>>

        <hspace|1.2ex>1<hspace|3ex><strong|if>
        <with|font-shape|small-caps|Stack-Empty><math|<around*|(|S|)>>

        <hspace|1.2ex>2<hspace|8ex><strong|error> ``underflow''

        <hspace|1.2ex>3<hspace|3ex><strong|else>

        <hspace|1.2ex>4<hspace|8ex><strong|if>
        <with|font-shape|small-caps|Queue-Empty><math|<around*|(|Q<rsub|2>|)>>

        <hspace|1.2ex>5<hspace|13ex><strong|while>
        <math|Q<rsub|1>.<text|<em|tail>> - Q<rsub|1>.<text|<em|head>> \<gtr\>
        1>

        <hspace|1.2ex>6<hspace|18ex><with|font-shape|small-caps|Enqueue><math|<around*|(|Q<rsub|2>,<text|<with|font-shape|small-caps|Dequeue>><around*|(|Q<rsub|1>|)>|)>>

        <hspace|1.2ex>7<hspace|13ex><with|font-shape|small-caps|Dequeue><math|<around*|(|Q<rsub|1>|)>>

        <hspace|1.2ex>8<hspace|8ex><strong|else>

        <hspace|1.2ex>9<hspace|13ex><strong|while>
        <math|Q<rsub|2>.<text|<em|tail>> - Q<rsub|2>.<text|<em|head>> \<gtr\>
        1>

        10<hspace|18ex><with|font-shape|small-caps|Enqueue><math|<around*|(|Q<rsub|1>,<text|<with|font-shape|small-caps|Dequeue>><around*|(|Q<rsub|2>|)>|)>>

        11<hspace|13ex><with|font-shape|small-caps|Dequeue><math|<around*|(|Q<rsub|2>|)>>
      </render-code>

      <hspace|3ex><with|font-shape|small-caps|Push> takes <math|O
      <around*|(|1|)>> time, while the running time of
      <with|font-shape|small-caps|Pop> has an order growth <math|O
      <around*|(|n|)>>.
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|exercise-indentation|0tab>
    <associate|font-base-size|12>
    <associate|indent-indentation|0tab>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?|../../../../../../../../../../.TeXmacs/texts/scratch/no_name_28.tm>>
    <associate|footnote-*|<tuple|?|?>>
  </collection>
</references>