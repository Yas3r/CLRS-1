<TeXmacs|1.99.1>

<style|generic>

<\body>
  <\render-exercise|10.1-6>
    <render-footnote|*|Creative Commons <image|<tuple|<#>|png>|1.20cm|0.42cm||>
    2014, Lawrence X. Amlord (<with|font|heiti|\<#989C\>\<#4E16\>\<#654F\>>,
    aka <with|font|heiti|\<#989C\> \<#5E8F\>>).<new-line>
    <hspace|2.20ex>Email address: informlarry@gmail.com>

    Show how to implement a quque using two stacks. Analyze the running time
    of the queue operations.

    <\answer>
      \;

      Using two stacks <math|S<rsub|1>> and <math|S<rsub|2>>, we can
      <with|font-shape|small-caps|Enqueue> an element to the queue <math|Q>
      by <with|font-shape|small-caps|Push>ing it into <math|S<rsub|1>> and
      <with|font-shape|small-caps|Dequeue> the tail element by
      <with|font-shape|small-caps|Pop>ing it from <math|S<rsub|2>>. If stack
      <math|S<rsub|2>> is empty when a <with|font-shape|small-caps|Dequeue>
      is requested, or <math|S<rsub|1>> is full when an
      <with|font-shape|small-caps|Enqueue> is requested, we transmit as many
      elements in <math|S<rsub|1>> to <math|S<rsub|2>> as possible. A queue
      is said to be empty if and only if both of its stacks <math|S<rsub|1>>
      and <math|S<rsub|2>> are empty. Likewise, the queue becomes full when
      <math|S<rsub|1>> and <math|S<rsub|2>> are both full.

      <\render-code>
        <with|font-shape|small-caps|Queue-Empty><math|<around*|(|Q|)>>

        1<hspace|3ex><strong|if> <with|font-shape|small-caps|Stack-Empty><math|<around*|(|S<rsub|1>|)>>
        and <with|font-shape|small-caps|Stack-Empty><math|<around*|(|S<rsub|2>|)>>

        2<hspace|8ex><strong|return> <with|font-shape|small-caps|true>

        3<hspace|3ex><strong|else> <strong|return>
        <with|font-shape|small-caps|false>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Queue-Full><math|<around*|(|Q|)>>

        1<hspace|3ex><strong|if> <with|font-shape|small-caps|Stack-Full><math|<around*|(|S<rsub|1>|)>>
        and <with|font-shape|small-caps|Stack-Full><math|<around*|(|S<rsub|2>|)>>

        2<hspace|8ex><strong|return> <with|font-shape|small-caps|true>

        3<hspace|3ex><strong|else> <strong|return>
        <with|font-shape|small-caps|false>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Enqueue><math|<around*|(|Q,x|)>>

        1<hspace|3ex><strong|if> <with|font-shape|small-caps|Queue-Full><math|<around*|(|Q|)>>

        2<hspace|8ex><strong|error> ``overflow''

        3<hspace|3ex><strong|else>

        4<hspace|8ex><strong|if> <with|font-shape|small-caps|Stack-Full><math|<around*|(|S<rsub|1>|)>>

        5<hspace|13ex><with|font-shape|small-caps|Transmit><math|<around*|(|S<rsub|1>,S<rsub|2>|)>>

        6<hspace|8ex><with|font-shape|small-caps|Push><math|<around*|(|S<rsub|1>,x|)>>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Dequeue><math|<around*|(|Q|)>>

        1<hspace|3ex><strong|if> <with|font-shape|small-caps|Queue-Empty><math|<around*|(|Q|)>>

        2<hspace|8ex><strong|error> ``underflow''

        3<hspace|3ex><strong|else>

        4<hspace|8ex><strong|if> <with|font-shape|small-caps|Stack-Empty><math|<around*|(|S<rsub|2>|)>>

        5<hspace|13ex><with|font-shape|small-caps|Transmit><math|<around*|(|S<rsub|1>,S<rsub|2>|)>>

        6<hspace|8ex><with|font-shape|small-caps|Pop><math|<around*|(|S<rsub|2>|)>>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Transmit><math|<around*|(|S<rsub|1>,S<rsub|2>|)>>

        1<hspace|3ex><strong|if> <with|font-shape|small-caps|Stack-Empty><math|<around*|(|S<rsub|2>|)>>

        2<hspace|8ex><with|font-shape|small-caps|Pour><math|<around*|(|S<rsub|1>,S<rsub|2>|)>>

        3<hspace|3ex><strong|else>

        4<hspace|8ex><math|T> = <with|font-shape|small-caps|Creat-Stack><math|<around*|(||)>>

        5<hspace|8ex><with|font-shape|small-caps|Pour><math|<around*|(|S<rsub|2>,T|)>>

        6<hspace|8ex><with|font-shape|small-caps|Pour><math|<around*|(|S<rsub|1>,T|)>>

        7<hspace|8ex><with|font-shape|small-caps|Pour><math|<around*|(|T,S<rsub|2>|)>>
      </render-code>

      <\render-code>
        <with|font-shape|small-caps|Pour><math|<around*|(|S<rsub|1>,S<rsub|2>|)>>

        1<hspace|3ex><strong|while> not <with|font-shape|small-caps|Stack-Full><math|<around*|(|S<rsub|2>|)>>

        2<hspace|8ex><with|font-shape|small-caps|Push><math|<around*|(|S<rsub|2>,<text|<with|font-shape|small-caps|Pop>><around*|(|S<rsub|1>|)>|)>>
      </render-code>

      <hspace|3ex>In the worst case, an <with|font-shape|small-caps|Enqueue>
      or <with|font-shape|small-caps|Dequeue> operation is performed when
      stack <math|S<rsub|1>> is full while stack <math|S<rsub|2>> is empty.
      We must transfer at most <math|n> elements in <math|S<rsub|1>> to
      <math|S<rsub|2>> and they both take time <math|O <around*|(|n|)>>.
      However, we could armotize the cost of the transfering over all the
      <with|font-shape|small-caps|Enqueue> and
      <with|font-shape|small-caps|Dequeue> operations. So the average-case
      running time of <with|font-shape|small-caps|Enqueue> and
      <with|font-shape|small-caps|Dequeue> takes time <math|O
      <around*|(|1|)>>.
    </answer>
  </render-exercise>
</body>

<\initial>
  <\collection>
    <associate|font-base-size|12>
    <associate|indent-indentation|0tab>
  </collection>
</initial>

<\references>
  <\collection>
    <associate|footnote-|<tuple|?|?|../../../../../../../../../../.TeXmacs/texts/scratch/no_name_28.tm>>
    <associate|footnote-*|<tuple|?|1>>
  </collection>
</references>